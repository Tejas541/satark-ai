...
const levelColor = { Low: "#48D597", Suspicious: "#F5C451", High: "#FF9500", Critical: "#FF6B35" } as const;
...
const riskLabels: Record<AnalysisLanguage, Record<ScamAnalysis["riskLevel"], string>> = {
  hindi: { Low: "कम", Suspicious: "संदिग्ध", High: "उच्च", Critical: "गंभीर" },
  marathi: { Low: "कमी", Suspicious: "संशयास्पद", High: "उच्च", Critical: "गंभीर" },
  english: { Low: "Low", Suspicious: "Suspicious", High: "High", Critical: "Critical" },
};
const urlVerificationLabels: Record<AnalysisLanguage, Record<"VERIFIED_THREAT" | "NO_THREAT_FOUND" | "VERIFICATION_UNAVAILABLE", string>> = {
  english: { VERIFIED_THREAT: "Threat detected", NO_THREAT_FOUND: "No known threat detected — this does not mean the message is safe.", VERIFICATION_UNAVAILABLE: "URL verification unavailable" },
  hindi: { VERIFIED_THREAT: "खतरा मिला", NO_THREAT_FOUND: "कोई ज्ञात खतरा नहीं मिला — इसका मतलब संदेश सुरक्षित है, यह नहीं है।", VERIFICATION_UNAVAILABLE: "URL सत्यापन उपलब्ध नहीं है" },
  marathi: { VERIFIED_THREAT: "धोका आढळला", NO_THREAT_FOUND: "कोणताही ज्ञात धोका आढळला नाही — याचा अर्थ संदेश सुरक्षित आहे असे नाही.", VERIFICATION_UNAVAILABLE: "URL पडताळणी उपलब्ध नाही" },
};
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
          ...
          {analysis ? <Animated.View style={[styles.resultCard, { opacity: cardOpacity, transform: [{ translateY: cardOpacity.interpolate({ inputRange: [0, 1], outputRange: [14, 0] }) }] }]}>
            ...
            {readOpen ? <>
              ...
              {analysis.urlFindings.length || analysis.urlVerification ? <View style={styles.section}><Text style={styles.dividerLabel}>{t.links}</Text>{analysis.urlVerification ? <View style={[styles.verificationState, analysis.urlVerification === "VERIFIED_THREAT" ? styles.verificationThreat : analysis.urlVerification === "VERIFICATION_UNAVAILABLE" ? styles.verificationUnavailable : styles.verificationNeutral]}><MaterialIcons name={analysis.urlVerification === "VERIFIED_THREAT" ? "gpp-bad" : analysis.urlVerification === "VERIFICATION_UNAVAILABLE" ? "help-outline" : "verified-user"} size={16} color={analysis.urlVerification === "VERIFIED_THREAT" ? "#FF8F70" : analysis.urlVerification === "VERIFICATION_UNAVAILABLE" ? "#F5C451" : "#A8A6B3"} /><Text style={[styles.verificationText, outputTextStyle]}>{urlVerificationLabels[outputLanguage][analysis.urlVerification]}</Text></View> : null}{analysis.urlFindings.map((finding) => <View key={finding} style={styles.finding}><MaterialIcons name="link" size={16} color="#F5C451" /><Text style={[styles.findingText, outputTextStyle]}>{finding}</Text></View>)}</View> : null}
...
const styles: Record<string, any> = StyleSheet.create({
  ...
  verificationState: { flexDirection: "row", alignItems: "flex-start", gap: 8, borderRadius: 12, padding: 10, marginBottom: 8, borderWidth: 1 },
  verificationThreat: { backgroundColor: "#2A1917", borderColor: "#5A2B21" },
  verificationUnavailable: { backgroundColor: "#26231A", borderColor: "#4A412A" },
  verificationNeutral: { backgroundColor: "#14141C", borderColor: "#292934" },
  verificationText: { color: "#D2D0D8", fontFamily: "Inter_500Medium", fontSize: 12.5, lineHeight: 18, flex: 1 },
